...
from database.db import (
    add_subscription,
    delete_subscription,
    stripe_subscription_for_user,
    config_for_payment_tiers,
    user_has_free_trial,
    stripe_customer_for_user,
    next_anchor_time_for_user,
)
import stripe
...
from database.db import (
    ...
    refresh_credits,
    user_email_for_id,
    user_email_for_customer_id,
    no_subscriptions_with_end_date_null,
)
...
def CreateCheckoutSessionHandler(request, userEmail):
    ...
    existing_subscription_id = stripe_subscription_for_user(
        userEmail
    )  # Adjust this function to return the subscription ID, not the session ID
...
def CreatePortalSessionHandler(request, userEmail):
    ...
    portalSession = stripe.billing_portal.Session.create(
        customer=customer_id, return_url=return_url, configuration=config
    )
...
def StripeWebhookHandler(request, event):
    ...
    if eventType == "checkout.session.completed":
        ...
        subscription_details = stripe.Subscription.retrieve(
        ...
        if subscription_details["trial_end"] is not None:
            free_trial_end = datetime.fromtimestamp(
        ...
        add_subscription(
            subscription_details, user_id, customer_id, payment_plan, free_trial_end
        )
        ...
        customer_id = subscription["customer"]
        cancellation_reason = (
            event["data"]["object"].get("cancellation_details", {}).get("reason")
        )
