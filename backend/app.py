...
from api_endpoints.financeGPT.chatbot_endpoints import (
    ...
    add_message_to_db,
    add_model_key_to_db,
    add_organization_to_db,
    add_prompt_to_workflow_db,
    add_sources_to_db,
    add_ticker_to_chat_db,
    add_workflow_to_db,
    change_chat_mode_db,
    check_valid_api,
    chunk_document,
    ...
    get_text_from_url,
    get_text_pages_from_single_file,
    process_prompt_answer,
    process_ticker_info_wf,
    remove_prompt_from_workflow_db,
    remove_ticker_from_workflow_db,
    reset_chat_db,
    reset_uploaded_docs,
...
def valid_api_key_required(fn):
    ...
    def wrapper(*args, **kwargs):
        ...
        # If API key is not present or valid
...
# Auth
# set environment to https because OAuth 2.0 only supports https environments
os.environ["OAUTHLIB_INSECURE_TRANSPORT"] = "1"

# enter your client id you got from Google console
GOOGLE_CLIENT_ID = (
    "261908856206-fff63nag7j793tkbapd3hugthbcp8kfn.apps.googleusercontent.com"
)
...
flow = Flow.from_client_secrets_file(
    ...
    # scopes below: what do we get after the authorization
    scopes=[
...
def create_organization():
    try:
        ...
        organization_type = request.json.get(
            "organization_type"
...
# Change this to use MYSQL db
@app.route("/api/reset-everything", methods=["POST"])
def reset_everything():
    try:
        # Delete vector database, shutil.rmtree(vector_base_path) & user input documents
...
def download_chat_history():
    ...
    try:
        ...
        for i in range(len(messages) - 1):
            if (
                messages[i]["sent_from_user"] == 1
                and messages[i + 1]["sent_from_user"] == 0
            ):
                regex = re.compile(
                    r"Document:\s*[^:]+:\s*(.*?)(?=Document:|$)", re.DOTALL
                )
...
def create_new_chat():
    ...
    chat_id = add_chat_to_db(
        user_email, chat_type, model_type
    )  # for now hardcode the model type as being 0
    ...
    try:
        ...
        for ticker in tickers:
            ...
            for question in questions:
                ...
                question_encoded = question.encode("latin-1", "replace").decode(
                    "latin-1"
                )
...
def upload():
    ...
    if chat_type == "documents":  # question-answering
        ...
        # create new chat
        model_number = (
            0 if model_type == "gpt" else 1 if model_type == "claude" else None
        )
        chat_number = (
            0 if chat_type == "documents" else 1 if chat_type == "edgar" else None
        )
...
def public_ingest_pdf():
    ...
    ensure_SDK_user_exists(
        user_email
    )  # do this here in case the user hasn't uploaded a document yet
    ...
    if model_type == 0:
        ...
        if model_key:
            return (
                jsonify(
                    {
                        "Error": "You cannot enter a fine-tuned model key when using Claude"
                    }
                ),
                400,
            )
    ...
    return jsonify(
        message_id=message_id, answer=answer, sources=sources_swapped
    )  # can modify the sources here if we don't want to return the sources
