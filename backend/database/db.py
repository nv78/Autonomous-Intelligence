...
def create_7_day_free_trial(user_id):
    ...
    cursor.execute(
        "INSERT INTO Subscriptions (stripe_info_id, subscription_id, end_date, paid_user, is_free_trial) VALUES (%s, %s, %s, 1, 1)",
        [stripe_info_id, "id", end_date],
    )
    conn.commit()
    conn.close()
...
def create_user_if_does_not_exist(email, google_id, person_name, profile_pic_url):
    ...
    if count is None or count["COUNT(*)"] == 0:
        ...
        cursor.execute(
            "INSERT INTO users (credits, email, google_id, person_name, profile_pic_url) VALUES (20, %s,%s,%s,%s)",
...
def create_user_from_credentials(email, password_hash, salt, session_token):
    ...
    cursor.execute(
        "INSERT INTO users (email, password_hash, session_token, session_token_expiration, salt) VALUES (%s, %s, %s, %s, %s)",
...
def update_session_token_for_user(email, session_token):
    ...
    expiration_limit = NOW + kSessionTokenExpirationTime
    cursor.execute(
        "UPDATE users SET session_token = %s , session_token_expiration = %s WHERE email = %s",
...
def update_user_credentials(email, hashed_password, salt, token):
    ...
    expiration_limit = NOW + kSessionTokenExpirationTime
    cursor.execute(
        "UPDATE users SET password_hash = %s, salt = %s, session_token = %s , session_token_expiration = %s WHERE email = %s",
...
def verify_password_reset_code(email, passwordResetCode):
    ...
    NOW = datetime.now()
    cursor.execute(
        "SELECT * FROM users WHERE email = %s AND password_reset_token = %s AND password_reset_token_expiration > %s",
...
def update_password_reset_token(email, generated_token):
    ...
    cursor.execute(
        "UPDATE users SET password_reset_token = %s , password_reset_token_expiration = %s WHERE email = %s",
...
def paid_user_for_user_email_with_cursor(conn, cursor, user_email):
    cursor.execute(
        "SELECT gc.paid_user FROM Subscriptions gc JOIN StripeInfo c ON c.id=gc.stripe_info_id JOIN users p ON p.id=c.user_id WHERE gc.start_date < CURRENT_TIMESTAMP AND (gc.end_date IS NULL OR gc.end_date > CURRENT_TIMESTAMP) AND p.email = %s ORDER BY gc.start_date ASC LIMIT 1",
        [user_email],
    )
...
def is_free_trial_for_user_email_with_cursor(conn, cursor, user_email):
    cursor.execute(
        "SELECT gc.is_free_trial FROM Subscriptions gc JOIN StripeInfo c ON c.id=gc.stripe_info_id JOIN users p ON p.id=c.user_id WHERE gc.start_date < CURRENT_TIMESTAMP AND (gc.end_date IS NULL OR gc.end_date > CURRENT_TIMESTAMP) AND p.email = %s ORDER BY gc.start_date ASC LIMIT 1",
        [user_email],
    )
    paidUser = cursor.fetchone()
    if paidUser:
...
def end_date_for_user_email_with_cursor(conn, cursor, user_email):
    cursor.execute(
        "SELECT gc.end_date FROM Subscriptions gc JOIN StripeInfo c ON c.id=gc.stripe_info_id JOIN users p ON p.id=c.user_id WHERE gc.start_date < CURRENT_TIMESTAMP AND (gc.end_date IS NULL OR gc.end_date > CURRENT_TIMESTAMP) AND p.email = %s ORDER BY gc.start_date ASC LIMIT 1",
        [user_email],
    )
    paidUser = cursor.fetchone()
    if paidUser:
...
def refresh_credits(user_email):
    ...
    cursor.execute(
        "SELECT c.anchor_date FROM StripeInfo c JOIN users p ON c.user_id=p.id WHERE p.id = %s",
        [result["id"]],
    )
    ...
    if (not anchorDateDb) or (not anchorDateDb["anchor_date"]):
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM Subscriptions gc JOIN StripeInfo c ON gc.stripe_info_id=c.id JOIN users p ON p.id=c.user_id WHERE p.email = %s AND gc.start_date < CURRENT_TIMESTAMP AND (gc.end_date IS NULL OR gc.end_date > CURRENT_TIMESTAMP)",
            (user_email,),
        )
        ...
        if noActiveSubscriptions:
            if result["credits"] > 0:
                cursor.execute(
                    "UPDATE users SET credits=0, credits_updated=CURRENT_TIMESTAMP WHERE id=%s",
                    [result["id"]],
                )
                conn.commit()
            conn.close()
            ...
            return {"numCredits": result["credits"]}
    ...
    if len(subscriptions) == 0:
        ...
        if sub["is_free_trial"] == 1:
            if (
                not result["credits_updated"]
                or sub["start_date"] >= result["credits_updated"]
            ):
                numCredits = planToCredits[sub["paid_user"]]
                cursor.execute(
                    "UPDATE users SET credits=%s, credits_updated=CURRENT_TIMESTAMP WHERE id=%s",
                    [numCredits, result["id"]],
                )
                conn.commit()
                conn.close()
                print("refresh_credits33")
                return {"numCredits": numCredits}
        else:
            if (
                not result["credits_updated"]
                or previousAnchorDate >= result["credits_updated"]
            ):
                numCredits = planToCredits[sub["paid_user"]]
                cursor.execute(
                    "UPDATE users SET credits=%s, credits_updated=CURRENT_TIMESTAMP WHERE id=%s",
...
def add_subscription(subscription, user_id, customer_id, payment_plan, free_trial_end):
    ...
    try:
        cursor.execute(
            "SELECT id, stripe_customer_id, anchor_date FROM StripeInfo WHERE user_id=%s",
            [user_id],
        )
        ...
        if stripe_info_id_db:
            ...
            if not stripe_info_id_db["stripe_customer_id"]:
                cursor.execute(
                    "UPDATE StripeInfo SET stripe_customer_id = %s WHERE id=%s",
                    [customer_id, stripe_info_id],
                )
        else:
            cursor.execute(
                "INSERT INTO StripeInfo (user_id, stripe_customer_id) VALUES (%s, %s)",
                (user_id, customer_id),
            )
            cursor.execute("SELECT LAST_INSERT_ID()")
            stripe_info_id = cursor.fetchone()["LAST_INSERT_ID()"]
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM Subscriptions c JOIN StripeInfo p ON c.stripe_info_id=p.id WHERE p.user_id=%s AND c.start_date < CURRENT_TIMESTAMP AND (c.end_date IS NULL OR c.end_date > CURRENT_TIMESTAMP)",
            [user_id],
        )
        ...
        if activePaidSubscriptions["COUNT(*)"] == 0:
            if free_trial_end:
                cursor.execute(
                    "UPDATE StripeInfo SET anchor_date = %s WHERE user_id = %s",
                    [free_trial_end, user_id],
                )
            else:
                cursor.execute(
                    "UPDATE StripeInfo SET anchor_date = CURRENT_TIMESTAMP WHERE user_id = %s",
                    [user_id],
                )
        elif not stripe_info_id_db["anchor_date"]:
            cursor.execute(
                "SELECT id FROM Subscriptions c JOIN StripeInfo p ON c.stripe_info_id=p.id WHERE p.user_id=%s AND c.start_date < CURRENT_TIMESTAMP AND (c.end_date IS NULL OR c.end_date > CURRENT_TIMESTAMP)",
                [user_id],
            )
            activePaidSubscriptionDbs = cursor.fetchall()
            for activePaidSubscriptionDb in activePaidSubscriptionDbs:
                cursor.execute(
                    "UPDATE Subscriptions SET end_date = CURRENT_TIMESTAMP WHERE id = %s",
                    [activePaidSubscriptionDb["id"]],
                )
            cursor.execute(
                "UPDATE StripeInfo SET anchor_date = CURRENT_TIMESTAMP WHERE user_id = %s",
                [user_id],
        ...
        if free_trial_end:
            cursor.execute(
                "INSERT INTO Subscriptions (stripe_info_id, subscription_id, paid_user, is_free_trial, end_date) VALUES (%s, %s, %s, %s, %s)",
                [
                    stripe_info_id,
                    subscription["id"],
                    int(payment_plan),
            ...
            cursor.execute(
                "INSERT INTO Subscriptions (stripe_info_id, subscription_id, paid_user, is_free_trial, start_date) VALUES (%s, %s, %s, %s, %s)",
                [
                    stripe_info_id,
                    subscription["id"],
                    int(payment_plan),
            ...
            cursor.execute(
                "INSERT INTO Subscriptions (stripe_info_id, subscription_id, paid_user, is_free_trial) VALUES (%s, %s, %s, %s)",
...
def delete_subscription(subscription):
    ...
    try:
        ...
        cursor.execute(
            "SELECT p.user_id FROM StripeInfo p JOIN Subscriptions c ON p.id=c.stripe_info_id WHERE c.subscription_id = %s LIMIT 1",
            [subscription["id"]],
        )
        ...
        cursor.execute(
            "SELECT is_free_trial FROM Subscriptions WHERE subscription_id = %s AND start_date < CURRENT_TIMESTAMP AND (end_date IS NULL OR end_date > CURRENT_TIMESTAMP) ORDER BY start_date DESC LIMIT 1",
            (subscription["id"],),
        )
...
def next_anchor_time_for_user_with_cursor(conn, cursor, user_id):
    cursor.execute(
        "SELECT c.anchor_date from StripeInfo c JOIN users p ON p.id=c.user_id WHERE p.id = %s",
        [user_id],
    )
    result = cursor.fetchone()
    ...
    if result and result["anchor_date"]:
        ...
        try:
            ...
            ValueError
        ):  # This will be triggered when the day isn't in the current month
            ...
            next_month_start = (now.replace(day=1) + relativedelta(months=1)).replace(
                hour=anchor_date.hour,
                minute=anchor_date.minute,
                second=anchor_date.second,
            )
        ...
        if now > target_date:
            try:
                ...
                next_month_start = (
                    now.replace(day=1) + relativedelta(months=2)
                ).replace(
                    hour=anchor_date.hour,
                    minute=anchor_date.minute,
                    second=anchor_date.second,
...
def view_user(user_email):
    ...
    cursor.execute(
        "SELECT c.paid_user FROM Subscriptions c JOIN StripeInfo p ON p.id=c.stripe_info_id WHERE p.user_id = %s AND c.end_date IS NULL AND c.start_date > CURRENT_TIMESTAMP ORDER BY c.start_date DESC LIMIT 1",
        [user["id"]],
    )
    next_plan = None
...
def user_has_free_trial(userEmail, free_trial_code):
    ...
    if freeTrialAllowlist:
        ...
        if freeTrialAllowlist:
            ...
            if (
                freeTrialAllowlistCount["COUNT(*)"]
                < freeTrialAllowlist["max_non_email_count"]
            ):
...
def user_email_for_customer_id(id):
    conn, cursor = get_db_connection()
    cursor.execute(
        "SELECT p.email FROM Subscriptions gc JOIN StripeInfo c ON c.id=gc.stripe_info_id JOIN users p ON p.id=c.user_id WHERE c.stripe_customer_id = %s",
        [id],
    )
    email = cursor.fetchone()
    cursor.close()
...
def no_subscriptions_with_end_date_null(user_email):
    conn, cursor = get_db_connection()
    cursor.execute(
        "SELECT COUNT(*) FROM Subscriptions gc JOIN StripeInfo c ON c.id=gc.stripe_info_id JOIN users p ON p.id=c.user_id WHERE p.email = %s AND gc.end_date IS NULL",
        [user_email],
    )
...
def check_and_debit_gpt_credit_with_cursor(conn, cursor, userEmail, numCredits):
    isOk = False
    cursor.execute(
        "SELECT num_chatgpt_requests, chat_gpt_date FROM users WHERE email=%s",
        [userEmail],
    )
    ...
    if (
        chatgpt_date.month < datetime.now().month
        and chatgpt_date.year <= datetime.now().year
    ) or (chatgpt_date.year < datetime.now().year):
        cursor.execute(
            "UPDATE users SET chat_gpt_date=%s, num_chatgpt_requests=1 WHERE email = %s",
            [datetime.now(), userEmail],
        )
        return True
    ...
    if num_chatgpt_requests <= chatgptLimit:
        isOk = True
        cursor.execute(
            "UPDATE users SET num_chatgpt_requests=%s WHERE email=%s",
            [num_chatgpt_requests + numCredits, userEmail],
        )
    return isOk
...
def generate_api_key(email):
    ...
    cursor.execute(
        "INSERT INTO apiKeys (user_id, api_key, created) VALUES (%s, %s, %s)",
        (userIdStr, api_key, time),
    )
    cursor.execute("SELECT LAST_INSERT_ID()")
    keyId = cursor.fetchone()["LAST_INSERT_ID()"]
    conn.commit()
    conn.close()
    return {"id": keyId, "key": api_key, "created": time, "last_used": None}
...
def get_api_keys(email):
    ...
    cursor.execute(
        "SELECT id, api_key, created, last_used FROM apiKeys WHERE user_id = %s",
        (userIdStr,),
    )
